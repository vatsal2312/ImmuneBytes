...
//    /**
//    * @dev Called by anyone to set their stake to a particular value explicitly.
//    * @dev Internally calls the increaseStake or decreaseStake function.
//    * @dev This performs an ERC20 transfer so the msg sender may need to grant approval to this contract before calling this function.
//    * @param amountToken The explicit value to set their stake to.
//    * @return success True if the operation completed successfully.
//    **/
//    function setStake(uint256 amountToken) external returns (bool success);
    ...
    * @return success True if the operation completed successfully.
    **/
    function openChallenge(bytes32 datasetHash, bytes32 keyHash) external returns (bool success);
    ...
    **/
    ...
    function updatePrivateKey(uint32 challengeNumber, bytes32 newKeyHash) external;
    ...
    **/
    ...
    /**
    * @dev Called only by authorized admin to move rewards from the competition pool to the participants' competition internal balances based on results from the current challenge.
    ...
    * @param rewards List of corresponding amount of tokens in wei won by each winner.
    * @return success True if operation completes successfully.
    **/
    function payStakingRewards(address[] calldata participants, uint256[] calldata rewards) external returns (bool success);

    /**
    * @dev Provides the same function as above but allows for challenge number to be specified.
    * @dev Note that the size of the array parameters passed in to this function is limited by the block gas limit.
    * @dev This function allows for the payout to be split into chunks by calling it repeatedly.
    * @param challengeNumber Challenge to make rewards payment for.
    * @param participants List of participants' addresses.
    * @param rewards List of corresponding amount of tokens in wei won by each winner.
    * @return success True if operation completes successfully.
    **/
    function payStakingRewards(uint32 challengeNumber, address[] calldata participants, uint256[] calldata rewards) external returns (bool success);
    ...
    * @dev Note that the size of the array parameters passed in to this function is limited by the block gas limit.
    * @dev This function allows for the payout to be split into chunks by calling it repeatedly.
    * @param winners List of winners' addresses.
    * @param challengeRewards List of corresponding amount of tokens in wei won by each winner for the challenge rewards portion.
    * @param tournamentRewards List of corresponding amount of tokens in wei won by each winner for the tournament rewards portion.
    * @return success True if operation completes successfully.
    **/
    function payChallengeAndTournamentRewards(address[] calldata winners, uint256[] calldata challengeRewards, uint256[] calldata tournamentRewards) external returns (bool success);
    ...
    * @return success True if operation completes successfully.
    **/
    function payChallengeAndTournamentRewards(uint32 challengeNumber, address[] calldata winners, uint256[] calldata challengeRewards, uint256[] calldata tournamentRewards) external returns (bool success);
    ...
    * @return success True if operation completes successfully.
    **/
    function updateInformationBatch(uint32 challengeNumber, address[] calldata participants, uint256 itemNumber, int[] calldata values) external returns (bool success);
    ...
    **/
    ...
    /**
    * @dev Called by anyone to check the total stakes locked for a particular challenge.
    * @param challengeNumber Challenge to get the total stakes locked of.
    * @return totalStakedForChallenge Total amount of tokens locked for this challenge.
    **/
    function getTotalStakesLockedForChallenge(uint32 challengeNumber) view external returns (uint256 totalStakedForChallenge);
    ...
    **/
    function getInformation(uint32 challengeNumber, address participant, uint256 itemNumber) view external returns (int value);
    ...
    **/
    ...
    /**
    * @dev Called by anyone to compute the staking reward allocation for this participant in the current challenge.
    * @param participant Address of participant to compute for.
    * @return stakingReward Staking reward allocation for this participant in the current challenge.
    **/
    function computeStakingReward(address participant) view external returns (uint256 stakingReward);
    ...
    **/
    ...
    event RewardsThresholdUpdated(uint256 indexed newThreshold);

    event StakeThresholdUpdated(uint256 indexed newStakeThreshold, address indexed adminAddress);
    ...
    event StakingRewardsPayment(address indexed participant, uint256 indexed rewardAmount);

    event StakingRewardsPaid(uint32 indexed challengeNumber, uint256 indexed totalAmount);

    event ChallengeAndTournamentRewardsPayment(address indexed winner, uint256 indexed challengeAmount, uint256 indexed tournamentAmount);

    event ChallengeAndTournamentRewardsPaid(uint32 indexed challengeNumber, uint256 indexed totalChallengeAmount, uint256 indexed totalTournamentAmount);
...
//    event SingleInformationUpdated(uint32 indexed challengeNumber, address indexed participant, uint256 indexed itemNumber, int value);

...
class CompetitionStateMachine(TokenStateMachine):
    ...
    def __init__(self, accounts, token_contract, comp_contract, proxy_admin, proxy):
    ...
    def rule_updateChallengeAndTournamentScores(self):
        ...
        for s in submitters:
            c_scores.append(int(Decimal(random.random()) * Decimal('1e18')))
            t_scores.append(int(Decimal(random.random()) * Decimal('1e18')))
...
def test_stateful_competition(state_machine, Token, Competition, ProxyAdmin, TransparentUpgradeableProxy, accounts):
    state_machine(CompetitionStateMachine, accounts, Token, Competition, ProxyAdmin, TransparentUpgradeableProxy)